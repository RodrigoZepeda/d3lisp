        ...
        (setf x (mapcar
                    (lambda (theta)
                        (+ (first center) (+ (* (- R1 R2) (cos theta))
                          (* d (cos (* (/ (- R1 R2) R2) theta))))))
                        angle-points))
        (setf y (mapcar
                    (lambda (theta)
                        (+ (second center) (+ (* (- R1 R2) (sin theta))
                          (* d (sin (* (/ (- R1 R2) R2) theta))))))
                        angle-points))
        (list x y)))
...
(defun hipocycloid (R1 R2 &key (center (list 0 0)) (angle-points (range 0 (* 5 pi))))
    (let (x y)
        (setf x (mapcar
                    (lambda (theta)
                        (+ (first center) (+ (* (- R1 R2) (cos theta))
                          (* R2 (cos (* (/ (- R1 R2) R2) theta))))))
                    angle-points))
        (setf y (mapcar
                    (lambda (theta)
                        (+ (second center) (+ (* (- R1 R2) (sin theta))
                        (* -1 R2 (sin (* (/ (- R1 R2) R2) theta))))))
                    angle-points))
        (list x y)))
